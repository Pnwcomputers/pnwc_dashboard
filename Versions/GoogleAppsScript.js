...
function sendConfirmationEmail(jobData) {
  try {
    // ✨ ADD LOGGING
    Logger.log('sendConfirmationEmail called with: ' + JSON.stringify(jobData));
    
    // ✨ DEFENSIVE: Check if required fields exist
    if (!jobData || !jobData.clientEmail) {
      Logger.log('Error: No client email provided');
      return;
    }
    
    const jobId = jobData.jobId || 'N/A';
    const clientName = jobData.clientName || 'Valued Customer';
    const serviceType = jobData.serviceType || 'N/A';
    const dueDate = jobData.dueDate || 'TBD';
    const currentStatus = jobData.currentStatus || 'N/A';
    const systemMake = jobData.systemMake || '';
    const initialRequest = jobData.initialRequest || '';
    
    const subject = `Service Confirmation - Job #${jobId} - Pacific NW Computers`;
    ...
    const htmlBody = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="padding: 30px; background-color: #f9fafb; border: 1px solid #e5e7eb;">
          <p style="font-size: 16px; color: #374151; margin-top: 0;">Dear ${clientName},</p>
          ...
          <div style="background-color: white; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #2563eb;">
            ...
            <table style="width: 100%; border-collapse: collapse;">
              <tr>
                <td style="padding: 8px 0; color: #6b7280; font-weight: 600;">Job ID:</td>
                <td style="padding: 8px 0; color: #1f2937; font-weight: bold;">${jobId}</td>
              </tr>
              <tr>
                <td style="padding: 8px 0; color: #6b7280; font-weight: 600;">Service Type:</td>
              ...
              <tr>
                <td style="padding: 8px 0; color: #6b7280; font-weight: 600;">Current Status:</td>
                <td style="padding: 8px 0; color: #1f2937;">${currentStatus}</td>
              </tr>
              ${systemMake ? `
              <tr>
                <td style="padding: 8px 0; color: #6b7280; font-weight: 600;">System:</td>
                <td style="padding: 8px 0; color: #1f2937;">${systemMake}</td>
              </tr>
              ` : ''}
            </table>
          </div>
          ...
          <div style="background-color: #eff6ff; padding: 15px; border-radius: 8px; margin: 20px 0;">
            <h3 style="color: #1e40af; margin-top: 0; font-size: 16px;">Your Request:</h3>
            <p style="color: #374151; margin-bottom: 0;">${initialRequest}</p>
          </div>
          ` : ''}
...
${systemMake ? 'System: ' + systemMake : ''}

${initialRequest ? 'YOUR REQUEST:\n' + initialRequest : ''}
...
Please keep this email for your records.
    ...
    Logger.log('Confirmation email sent successfully to: ' + jobData.clientEmail);
  } catch (error) {
    Logger.log('Error sending confirmation email: ' + error.message);
